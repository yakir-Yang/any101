/*
 * This source code was worte by Jason Ish's tailq-example item. Copy from link:
 *   https://github.com/jasonish/tailq-example/blob/master/tailq_ex.c
 */
 ...
 * TAILQ_HEAD macro.
 */
static TAILQ_HEAD(, tailq_entry) my_tailq_head;
	...
	TAILQ_FOREACH(item, &my_tailq_head, entries) {
		if (item->value == 5) {
			struct tailq_entry *new_item =
			    (struct tailq_entry *)malloc(sizeof(*new_item));
			if (new_item == NULL) {
				perror("malloc failed");
				exit(EXIT_FAILURE);
			}
